...
function CreditExhaustionCard({
  ...
  actions,
  onAction,
  // onOpenWhatsAppDropdown // Commented out for now
}: {
  message: string;
  actions: ChatMessage["actions"];
  onAction: (type: string) => void;
  // onOpenWhatsAppDropdown?: (anchorEl: HTMLElement, originalQuery?: string) => void; // Commented out for now
}) {
  return (
    <div className="credit-exhaustion-card bg-gradient-to-br from-orange-50 to-amber-50 rounded-2xl border border-orange-200 p-5 my-3 mx-2">
      ...
      <div className="flex flex-wrap gap-2">
        {actions?.map((action, idx) => {
          ...
          // else if (action.type === "whatsapp") { // Commented out for now
          ...
          return (
            <button
              key={idx}
              onClick={() => {
                // if (action.type === 'whatsapp') { // Commented out for now
                //   e.stopPropagation();
...
export function ChatMessages({
  ...
  const handleSupportAction = (action: string) => {
    ...
    if (action === 'contact_support') {
      // Open contact support (could be WhatsApp, email, or custom modal) - COMMENTED OUT FOR NOW
      // window.open('https://api.whatsapp.com/send/?phone=4407727449124&text&type=phone_number&app_absent=0', '_blank');
      console.log('[ChatMessages] Contact support clicked - WhatsApp option is currently disabled');
            ...
            <div className={`flex-1 ${msg.role === "assistant" ? "flex flex-col items-end" : ""}`}>
              ...
              {msg.role === "assistant" && msg.isCreditExhausted && msg.actions && msg.actions.length > 0 && (
                <CreditExhaustionCard
                  ...
                  // onOpenWhatsAppDropdown={(el) => onOpenWhatsAppDropdown?.(el)} // Commented out for now
                />
              )}
              ...
              {msg.role === "assistant" && msg.actions && msg.actions.length > 0 && !msg.isCreditExhausted && (
                ...
                {msg.actions.map((action, actionIdx) => {
                  ...
                  //   );
                  // }
                  if (action.type === "hubspot_chat" || action.type === "chat_with_us") {
        ...
        <div className="support-buttons-card mx-2 my-4 animate-fade-in">
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-blue-200 overflow-hidden">
            ...
            <div className="p-5 space-y-3">
              ...
              {/* COMMENTED OUT FOR NOW - WhatsApp Support Button */}
              {/* <button
